...
class Processor():
    ...
    def set_optimizer(self):
        ...
        self.criterion = nn.MSELoss(reduction=False)
    ...
    def test_epoch(self,epoch):
        ...
        for batch in range(self.dataloader_gt.testbatchnums):
            ...
            for pre_tra in full_pre_tra:
                error, error_cnt, final_error, final_error_cnt, first_erro,first_erro_cnt = \
                L2forTest(pre_tra, batch_norm_gt[1:, :, :2],self.args.obs_length)   # error = N , final_error = N, first_erro = N, where N is the num of pedestrians
                error_epoch_list.append(torch.sum(error))
                final_error_epoch_list.append(torch.sum(final_error))  #Summed over all pedestrians
                first_erro_epoch_list.append(torch.sum(first_erro))
            # print(error_epoch_list, final_error_epoch_list, first_erro_epoch_list)

            # all_error+=error_epoch_list
            # all_final_error+=final_error_epoch_list
            # all_first_error+=first_erro_epoch_list


            first_erro_epoch += min(first_erro_epoch_list)  # Minimum over best predictions
            final_error_epoch += min(final_error_epoch_list)
            error_epoch += min(error_epoch_list)
            
            all_error.append(min(error_epoch_list))  # Error at each epoch
            all_final_error.append(min(final_error_epoch_list)) # Final error at each epoch
            all_first_error.append(min(first_erro_epoch_list)) # First error at each epoch
            ...
            first_erro_cnt_epoch += first_erro_cnt
            error_epoch_list, final_error_epoch_list, first_erro_epoch_list = [], [], []
        total_times = len(all_error)
        all_error = [np.round(x.item()*total_times/error_cnt_epoch,3) for x in all_error]  # ADE at different time
        all_final_error = [np.round(x.item()*total_times/final_error_cnt_epoch,3) for x in all_final_error]  # FDE at different time
        all_first_error = [np.round(x.item()*total_times/first_erro_cnt_epoch,3) for x in all_first_error] # First error at different time 

        sorted_all_error = sorted(all_error)
        sorted_all_final_error = sorted(all_final_error)
        sorted_all_first_error = sorted(all_first_error)
        plotter(sorted_all_error,sorted_all_final_error)


        # print(all_error, all_final_error, all_first_error)
