...
class DataLoader_bytrajec2():
    def __init__(self, args,is_gt=True):
        ...
        if self.args.dataset=='eth5':
        ...
        elif self.args.dataset=='citysim':
            self.data_dirs = ['./data/Citysim_debug/Intersection_B__Non-Signalized_Intersection_/Trajectories/IntersectionB-01_gatraj.csv', './data/Citysim_debug/Intersection_B__Non-Signalized_Intersection_/Trajectories/IntersectionB-02_gatraj.csv'] 
            self.data_dir = './data'
            if args.ifvalid:
                self.val_fraction = args.val_fraction
            else:
            ...
            skip = [15, 15] #This is the difference between frames in each file
            self.train_dir=[self.data_dirs[x] for x in train_set]
            self.test_dir = [self.data_dirs[x] for x in self.test_set]
            self.trainskip=[skip[x] for x in train_set]
            self.testskip=[skip[x] for x in self.test_set]
    ...
    def get_data_index(self,data_dict,setname,ifshuffle=False):
        '''
...
def L2forTest(outputs,targets,obs_length):
    ...
    error = torch.sum(error_pred_length, dim=0)
    error_cnt = error_pred_length.numel()
    if torch.sum(error).item() == 0:
        return 0,0,0,0,0,0
    final_error = error_pred_length[-1]
    final_error_cnt = error_pred_length[-1].numel()
    first_erro = error_pred_length[0]
    first_erro_cnt = error_pred_length[0].numel()
    return error,error_cnt,final_error,final_error_cnt,first_erro,first_erro_cnt
